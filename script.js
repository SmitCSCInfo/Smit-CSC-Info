...
// content-section.js માં
async function loadContentFiles() {
    const response = await fetch('http://localhost:3000/api/content/files');
    const files = await response.json();
    
    files.forEach(file => {
      // HTML જનરેટ કરો અને DOM માં ઉમેરો
      const fileElement = `
      ...
      `;
      document.getElementById('content-container').innerHTML += fileElement;
    });
  }